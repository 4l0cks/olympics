...
def test_indiv_medals():
    rows = db.get_individual_medals()
    assert len(rows) > 100

def test_indiv_medals_id():
...
def test_countries_no_id():
    rows = db.get_countries()
    assert len(rows) > 0


def test_athletes_no_id():
    rows = db.get_athletes()
    assert len(rows) > 0


def test_disciplines_no_id():
    rows = db.get_disciplines()
    assert len(rows) > 0


def test_teams_no_id():
    rows = db.get_teams()
    assert len(rows) > 0


def test_events_no_id():
    rows = db.get_events()
    assert len(rows) > 0


def test_medals_no_id():
    rows = db.get_medals()
    assert len(rows) > 0


def test_discipline_athletes_no_id():
    rows = db.get_discipline_athletes(1)
    assert len(rows) > 0
...
def test_collective_medals_no_id():
    rows = db.get_collective_medals()
    assert len(rows) > 0


def test_top_collective_no_top():
    rows = db.get_top_collective()
    assert len(rows) == 10


def test_individual_medals_no_id():
    rows = db.get_individual_medals()
    assert len(rows) > 0


def test_top_individual_no_top():
